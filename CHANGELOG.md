...
- **設定檔格式升級**：`config.json` 採用新的多設定檔架構，支援為不同地點建立獨立設定檔。
  - **向後兼容**：舊版 config.json 會在首次啟動時自動轉換為新格式，建立名為「預設」的設定檔。
  - 新格式包含 `global`（全局設定）和 `profiles`（設定檔陣列）兩大區塊。
- **長按按鈕行為變更**：長按按鈕不再清除設定檔，改為進入 AP 模式（保留所有設定檔）。
  - 長按 5 秒 → 設定 `force_ap_mode` 標記 → 重啟並進入 AP 模式。
  - 所有設定檔都會保留，可透過網頁介面管理。

### 新增功能 (Added)
- **🆕 多設定檔支援**：
  - 可建立多個設定檔，每個對應不同地點（如家裡、公司、咖啡廳）。
  - 每個設定檔包含獨立的 WiFi、天氣地點、光感臨界值、圖片間隔、響聲設定等。
  - 全局設定（AP Mode、Weather API Key）在所有設定檔間共用。
- **🔍 智能 WiFi 連接邏輯**：
  - 啟動時自動掃描網路並獲取信號強度（RSSI）。
  - 根據 WiFi SSID 自動匹配對應的設定檔。
  - 連接優先順序：
    1. 優先嘗試上次成功連接的設定檔
    2. 其他設定檔按信號強度（RSSI）排序嘗試
  - 連接成功後自動設定為活動設定檔。
- **🖥️ 網頁端設定檔管理 UI**：
  - 雙欄式界面：左側設定檔列表 + 右側編輯區域。
  - 設定檔操作：
    - ➕ 新增設定檔（複製當前設定檔的非 WiFi 設定）
    - ✏️ 編輯設定檔（修改所有參數）
    - 🗑️ 刪除設定檔（至少需保留一個）
  - 🔵 藍點標示當前活動設定檔。
  - 即時顯示光感應器數值，每 3 秒更新。
- **🔥 完全重置功能**：
  - 在網頁介面「⚠️ 危險區域」提供完全重置選項。
  - 雙重安全確認：
    1. 輸入 `RESET` 文字驗證
    2. 再次確認對話框
  - 執行後刪除所有設定檔並恢復出廠設定。
- **📱 AP 模式超時改進**：
  - 超時後不再單純重啟，改為切換到最後連接的設定檔並重啟。
  - 確保即使 AP 模式超時，系統仍能使用已知設定運行。
- **📖 完整文件**：
  - 新增 `CONFIG_GUIDE.md` 詳細說明設定檔格式、使用方式、常見問題。
  - 更新 `config.json.example` 展示多設定檔範例。

### 變更與重構 (Changed & Refactored)
- **`config_manager.py` 完全重構**：
  - 新增設定檔管理方法：
    - `list_profiles()`, `get_profile()`, `add_profile()`, `update_profile()`, `delete_profile()`
    - `find_profile_by_ssid()` - 根據 WiFi SSID 尋找對應設定檔
    - `set_active_profile()`, `set_last_connected_profile()`
  - 保留向後兼容的 `get()` 和 `set()` 方法，現有程式碼無需修改。
  - 自動遷移邏輯：偵測舊格式並轉換為新格式。
- **`wifi_manager.py` 智能連接與管理**：
  - `wifi_manager()` 函數新增 `force_ap_mode` 檢查邏輯。
  - `scan_networks()` 函數回傳信號強度（RSSI）資訊。
  - 新增 `factory_reset()` 函數處理完全重置。
  - 網頁 HTML 重構為設定檔管理介面。
  - 新增 API endpoints：
    - `/edit_profile?name=...` - 編輯指定設定檔
    - `/new_profile?name=...` - 建立新設定檔
    - `/delete_profile?name=...` - 刪除設定檔
    - `/save_profile` - 儲存設定檔並重啟
    - `/factory_reset` - 執行完全重置
- **`reset_wifi_and_reboot()` 函數行為變更**：
  - 不再清空設定，改為設定 `force_ap_mode` 標記。
  - 重啟後進入 AP 模式，所有設定檔保持完整。
- **設定檔識別機制**：
  - 使用 WiFi SSID 作為設定檔識別依據。
  - 同一個 SSID 只應對應一個設定檔。

### 技術改進 (Improvements)
- **記憶體效率**：分段處理設定檔資料，降低記憶體使用。
- **錯誤處理**：增強設定檔操作的異常處理機制。
- **使用者體驗**：
  - 清晰的視覺分隔（危險區域使用紅色警告）。
  - 詳細的操作提示與確認機制。
...
### 文件更新 (Documentation)
- 更新 `README.md` 反映多設定檔功能。
- 更新 `config.json.example` 為新格式。
- 新增 `CONFIG_GUIDE.md` 完整設定檔說明文件。
- 更新所有相關說明，包含長按按鈕行為、AP 模式進入方式等。
